    ...
    return (
        ...
        >
            <div className="desktop-shift-contact-header">
                <motion.div
                    className="max-w-7xl mx-auto text-center mb-32 md:mb-48"
                    initial="hidden"
                    whileInView="visible"
                ...
                >
                    <motion.h2
                        ...
                        className="text-3xl md:text-4xl lg:text-5xl font-bold mb-8 uppercase tracking-tighter"
                    >
                        Contactos
                    </motion.h2>
                    <div style={{ transform: 'translateY(0.2cm)' }}>
                        <motion.p
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[0.85fr_1.15fr] gap-12 lg:gap-24 items-center">
                {/* Left Side: Contact Info */}
                <div className="flex-1 flex flex-col justify-center gap-10 desktop-shift-contact-info">
                    {contactInfo.map((item, index) => (
                        <div key={index} className={item.label === "EMAIL" ? "contact-email-row-wrapper" : "contact-standard-row-wrapper"}>
                            <motion.div
                            ...
                            >
                                <div className="text-primary mt-0.5 contact-icon-wrapper">{item.icon}</div>
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-xs font-bold tracking-widest text-primary uppercase">{item.label}</span>
                                    <span className={`text-xl font-light leading-snug whitespace-pre-line text-text contact-value-box ${item.label === "EMAIL" ? "contact-email-box" : ""}`}>
                                        {item.value}
                                    </span>
                                </div>
                            </motion.div>
                ...
                {/* Right Side: Map */}
                <div
                    className="h-[300px] lg:h-[350px] rounded-3xl overflow-hidden shadow-2xl relative border border-white/5 bg-slate-800 desktop-shift-contact-map"
                >
                    <iframe
                        title="Location Map"
            ...
            <div className="desktop-shift-contact-form">
                <motion.div
